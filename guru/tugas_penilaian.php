...
<?php foreach($daftar_siswa as $s): ?><tr <?= $fokus_id===(int)$s['pengumpulan_id']?'class="table-warning"':'' ?>><td><strong><?= sanitize($s['nama_lengkap']) ?></strong><br><small class="text-muted">NIS <?= sanitize($s['nis'] ?: '-') ?> · NISN <?= sanitize($s['nisn'] ?: '-') ?></small></td><td><?php if($s['pengumpulan_id']): ?><span class="badge bg-success">Terkumpul</span><?php else: ?><span class="badge bg-secondary">Belum</span><?php endif; ?></td><td class="small"><?= $s['dikumpulkan_pada']?date('d/m/Y H:i',strtotime($s['dikumpulkan_pada'])):'—' ?></td><td><?php if($s['file_tugas']): ?><?php $ext_file = strtolower(pathinfo($s['file_tugas'], PATHINFO_EXTENSION)); $bisa_preview = in_array($ext_file, ['pdf','png','jpg','jpeg'], true); ?><div class="d-flex flex-wrap gap-1"><?php if($bisa_preview): ?><button type="button" class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#modalPreview" onclick='previewJawaban(<?= json_encode("tugas_file.php?id=" . (int)$s["pengumpulan_id"] . "&mode=preview", JSON_HEX_APOS|JSON_HEX_QUOT) ?>, <?= json_encode($s["nama_lengkap"], JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP) ?>)'><i class="fa-solid fa-eye me-1"></i> Preview</button><?php endif; ?><a class="btn btn-sm btn-outline-primary" href="tugas_file.php?id=<?= (int)$s['pengumpulan_id'] ?>&amp;mode=download"><i class="fa-solid fa-download me-1"></i> Unduh</a></div><?php else: ?>—<?php endif; ?></td><td><?= $s['nilai']!==null?'<strong>'.number_format((float)$s['nilai'],2,',','.').'</strong>':'<span class="text-muted">Belum</span>' ?></td><td><?php if($s['pengumpulan_id']): ?><button class="btn btn-sm btn-warning text-white" data-bs-toggle="modal" data-bs-target="#modalNilai" onclick='isiNilai(<?= json_encode(["id"=>(int)$s["pengumpulan_id"],"nama"=>$s["nama_lengkap"],"nilai"=>$s["nilai"],"catatan"=>$s["catatan_guru"],"catatan_siswa"=>$s["catatan"]],JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP) ?>)'><i class="fa-solid fa-pen me-1"></i> Nilai</button><?php else: ?><button class="btn btn-sm btn-light" disabled>Belum mengirim</button><?php endif; ?></td></tr><?php endforeach; ?>
...
<div class="modal fade" id="modalPreview" tabindex="-1" aria-labelledby="judulPreview" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-centered"><div class="modal-content"><div class="modal-header"><div><h5 class="modal-title fw-bold" id="judulPreview">Preview Jawaban</h5><small class="text-muted" id="namaPreview"></small></div><button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Tutup"></button></div><div class="modal-body p-0 bg-light"><iframe id="framePreview" title="Preview jawaban siswa" style="width:100%;height:75vh;border:0;background:#fff"></iframe></div><div class="modal-footer"><span class="small text-muted me-auto"><i class="fa-solid fa-eye me-1"></i> Mode hanya lihat</span><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button></div></div></div></div>
...
function previewJawaban(url,nama){document.getElementById('namaPreview').textContent='Jawaban: '+nama;document.getElementById('framePreview').src=url;}
document.getElementById('modalPreview').addEventListener('hidden.bs.modal',function(){document.getElementById('framePreview').src='about:blank';});
</script>
