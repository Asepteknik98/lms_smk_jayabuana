...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($aksi === 'create' || $aksi === 'update') {
            ...
            $metode_pengumpulan = (string)($_POST['metode_pengumpulan'] ?? 'online');
            $deadline_raw = trim($_POST['deadline'] ?? '');
            $deadline_obj = DateTime::createFromFormat('Y-m-d\TH:i', $deadline_raw);

            if (!$pengajaran_ids || $pertemuan_ke < 1 || $pertemuan_ke > 20 || $judul === '' || !$deadline_obj || !in_array($jenis_tugas, ['portofolio','esai','pilihan_ganda','merangkum','video'], true) || !in_array($metode_pengumpulan, ['online','offline'], true)) {
            ...
            if ($metode_pengumpulan === 'offline') $jenis_tugas = 'portofolio';
            ...
            if ($aksi === 'create') {
                ...
                $db->beginTransaction();
                $sql = 'INSERT INTO tugas (pengajaran_id, pertemuan_ke, judul, deskripsi, jenis_tugas, metode_pengumpulan, deadline, file_lampiran) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
                ...
                foreach ($pengajaran_ids as $index => $id_pengajaran) {
                    ...
                    $buat_tugas->execute([$id_pengajaran, $pertemuan_ke, $judul, $deskripsi ?: null, $jenis_tugas, $metode_pengumpulan, $deadline_obj->format('Y-m-d H:i:s'), $lampiran_kelas]);
                ...
                kembali_tugas('success', $metode_pengumpulan === 'offline'
                    ? "Tugas offline berhasil dibuat untuk $jumlah_kelas kelas. Nilai dapat diinput oleh guru."
                    : "Tugas berhasil dibuat untuk $jumlah_kelas kelas dan dapat dilihat siswa.");
            }

            $cek = $db->prepare('SELECT t.file_lampiran,t.jenis_tugas,t.metode_pengumpulan,(SELECT COUNT(*) FROM pengumpulan_tugas pt WHERE pt.tugas_id=t.id) total_pengumpulan FROM tugas t JOIN pengajaran p ON p.id=t.pengajaran_id WHERE t.id=? AND p.guru_id=?');
            ...
            if ($struktur_dikunci) {
                $jenis_tugas = $lama['jenis_tugas'];
                $metode_pengumpulan = $lama['metode_pengumpulan'];
            }
            $db->beginTransaction();
            $sql = 'UPDATE tugas SET pengajaran_id=?, pertemuan_ke=?, judul=?, deskripsi=?, jenis_tugas=?, metode_pengumpulan=?, deadline=?, file_lampiran=? WHERE id=?';
            $db->prepare($sql)->execute([$pengajaran_id, $pertemuan_ke, $judul, $deskripsi ?: null, $jenis_tugas, $metode_pengumpulan, $deadline_obj->format('Y-m-d H:i:s'), $lampiran, $id]);
        ...
        <?php else: ?>
            ...
            <?php foreach ($daftar_tugas as $t): $lewat = strtotime($t['deadline']) < time(); ?>
                <div class="col-12 col-md-6 col-xl-4"><article class="card teacher-task-card h-100"><div class="card-body p-3 d-flex flex-column">
                    ...
                    <div class="d-flex flex-wrap gap-1 mb-2"><span class="badge bg-info-subtle text-info"><i class="fa-solid fa-shapes me-1"></i><?= sanitize($label_jenis[$t['jenis_tugas']] ?? 'Portofolio') ?></span><span class="badge <?= $t['metode_pengumpulan']==='offline'?'bg-warning-subtle text-warning':'bg-success-subtle text-success' ?>"><i class="fa-solid <?= $t['metode_pengumpulan']==='offline'?'fa-pen-to-square':'fa-globe' ?> me-1"></i><?= $t['metode_pengumpulan']==='offline'?'Offline':'Online' ?></span></div>
                    ...
                    <div class="task-stats mb-3"><div><strong><?= (int)$t['total_siswa'] ?></strong><small>Siswa</small></div><div><strong class="text-success"><?= (int)$t['sudah_mengumpulkan'] ?></strong><small><?= $t['metode_pengumpulan']==='offline'?'Dinilai':'Terkumpul' ?></small></div><div><strong class="text-warning"><?= $t['metode_pengumpulan']==='offline'?max(0,(int)$t['total_siswa']-(int)$t['sudah_mengumpulkan']):(int)$t['belum_dinilai'] ?></strong><small>Belum Nilai</small></div></div>
                    <div class="d-flex gap-2 mt-auto"><a href="<?= $t['metode_pengumpulan']==='offline'?'tugas_nilai_offline.php':'tugas_penilaian.php' ?>?tugas_id=<?= (int)$t['id'] ?>" class="btn btn-primary btn-sm task-action-main flex-grow-1"><i class="fa-solid fa-users-viewfinder me-1"></i><?= $t['metode_pengumpulan']==='offline'?'Input Nilai Offline':'Lihat & Nilai' ?></a><button class="btn btn-outline-secondary btn-sm task-action-icon" data-bs-toggle="modal" data-bs-target="#modalTugas" aria-label="Edit tugas" onclick='editTugas(<?= json_encode(["id"=>(int)$t["id"],"pengajaran_id"=>(int)$t["pengajaran_id"],"pertemuan_ke"=>(int)$t["pertemuan_ke"],"judul"=>$t["judul"],"deskripsi"=>$t["deskripsi"],"jenis_tugas"=>$t["jenis_tugas"],"metode_pengumpulan"=>$t["metode_pengumpulan"],"struktur"=>$t["struktur"],"terkumpul"=>(int)$t["sudah_mengumpulkan"],"deadline"=>date("Y-m-d\\TH:i",strtotime($t["deadline"]))], JSON_HEX_APOS|JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP) ?>)'><i class="fa-solid fa-pen"></i></button><form method="post" class="d-flex" onsubmit="return confirm('Hapus tugas beserta seluruh jawaban siswa?')"><input type="hidden" name="csrf_token" value="<?= sanitize($_SESSION['csrf_token']) ?>"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int)$t['id'] ?>"><button class="btn btn-outline-danger btn-sm task-action-icon" aria-label="Hapus tugas"><i class="fa-solid fa-trash"></i></button></form></div>
                </div></article></div>
            <?php endforeach; ?>
            </div>
...
<div class="modal fade" id="modalTugas" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable"><div class="modal-content border-0"><form method="post" enctype="multipart/form-data" id="formTugas"><div class="modal-header"><div><h5 class="modal-title fw-bold" id="judulModal">Buat Tugas Baru</h5><small class="text-muted">Lengkapi informasi tugas untuk siswa.</small></div><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body">
    ...
    <div class="col-md-6"><label class="form-label fw-semibold">Tenggat / Tanggal Masuk Rekap</label><input type="datetime-local" class="form-control" name="deadline" id="deadlineTugas" required><small class="text-muted">Nilai dihitung dalam rekap setelah waktu ini.</small></div><div class="col-md-6"><label class="form-label fw-semibold">Lampiran</label><input type="file" class="form-control" name="file_lampiran" accept=".pdf,.docx,.pptx,.zip"><small class="text-muted">PDF, DOCX, PPTX, ZIP · maks. 10 MB</small></div></div>
...
const jenisTugas=document.getElementById('jenisTugas'),metodePengumpulan=document.getElementById('metodePengumpulan'),kolomJenisTugas=document.getElementById('kolomJenisTugas'),areaPertanyaan=document.getElementById('areaPertanyaan'),daftarPertanyaan=document.getElementById('daftarPertanyaan');
