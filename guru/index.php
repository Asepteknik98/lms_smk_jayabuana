...
<style>
    ...
    .dashboard-section-head { padding-bottom:12px; border-bottom:1px solid #eef2f6; }
    .dashboard-section-icon { width:32px; height:32px; border-radius:9px; display:grid; place-items:center; flex:0 0 32px; font-size:.8rem; }
    .teaching-groups { display:grid; gap:8px; }
    .teaching-group { border:1px solid #e8edf3; border-radius:12px; overflow:hidden; }
    .teaching-group summary { padding:11px 12px; cursor:pointer; list-style:none; display:flex; align-items:center; justify-content:space-between; gap:10px; }
    .teaching-group summary::-webkit-details-marker { display:none; }
    .teaching-group summary::after { content:"+"; width:24px; height:24px; border-radius:7px; background:#f1f5f9; color:#64748b; display:grid; place-items:center; flex:0 0 24px; }
    .teaching-group[open] summary::after { content:"−"; }
    .teaching-group-title { min-width:0; }
    .teaching-group-title strong { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#334155; font-size:.78rem; }
    .teaching-group-title small { display:block; margin-top:2px; color:#94a3b8; font-size:.67rem; }
    .teaching-list { padding:0 11px 8px; border-top:1px solid #f1f4f7; }
    .teaching-row { padding:9px 1px; border-bottom:1px solid #f1f4f7; display:grid; grid-template-columns:minmax(0,1fr) auto auto; align-items:center; gap:10px; }
    .teaching-row:last-child { border-bottom:0; }
    .teaching-row-class { min-width:0; }
    .teaching-row-class strong { display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:.75rem; color:#334155; }
    .teaching-row-class small,.teaching-student-count { color:#94a3b8; font-size:.66rem; }
    .teaching-row .dropdown-toggle { min-height:31px; padding:.25rem .55rem; font-size:.7rem; border-color:#dbe3ec; color:#475569; }
    .grading-list { display:grid; gap:4px; }
    .grading-compact { padding:10px 0; border-bottom:1px solid #f1f4f7; display:flex; align-items:center; gap:9px; }
    .grading-compact:last-child { border-bottom:0; }
    .grading-compact-icon { width:30px; height:30px; border-radius:9px; display:grid; place-items:center; flex:0 0 30px; font-size:.72rem; }
    .grading-compact .btn { min-height:31px; padding:.25rem .55rem; font-size:.7rem; }
    .grading-empty { min-height:150px; display:grid; place-items:center; text-align:center; color:#64748b; }
    .grading-empty-icon { width:38px; height:38px; margin:0 auto 9px; border-radius:50%; display:grid; place-items:center; }
    ...
    @media (max-width:575.98px) {
        ...
        .teaching-row { grid-template-columns:minmax(0,1fr) auto; }
        .teaching-student-count { grid-column:1; }
        .teaching-row .dropdown { grid-column:2; grid-row:1/3; }
    }
</style>
<div id="page-content-wrapper" class="teacher-dashboard">
    ...
    <main class="teacher-main p-3 p-md-4">
        ...
        <div class="row g-3">
            <div class="col-lg-7 teacher-content-primary" id="pengajaran-saya"><section class="card teacher-panel h-100"><div class="card-body p-4"><div class="dashboard-section-head d-flex align-items-center gap-2 mb-3"><span class="dashboard-section-icon bg-primary-subtle text-primary"><i class="fa-solid fa-book-open"></i></span><div><h2 class="h6 fw-bold mb-0">Pengajaran Saya</h2><small class="text-muted"><?= count($daftar_pengajaran) ?> kelas yang diampu</small></div></div><?php if(!$pengajaran_per_mapel): ?><div class="text-center py-4 text-muted small">Belum ada kelas atau mata pelajaran dari admin.</div><?php else: ?><div class="teaching-groups"><?php foreach($pengajaran_per_mapel as $nama_mapel=>$daftar_kelas): ?><details class="teaching-group"><summary><span class="teaching-group-title"><strong><?= sanitize($nama_mapel) ?></strong><small><?= count($daftar_kelas) ?> kelas</small></span></summary><div class="teaching-list"><?php foreach($daftar_kelas as $p): ?><div class="teaching-row"><span class="teaching-row-class"><strong><?= sanitize($p['nama_kelas']) ?></strong><small><?= (int)$p['total_siswa'] ?> siswa</small></span><span class="teaching-student-count"><?= (int)$p['total_siswa'] ?> siswa</span><div class="dropdown"><button class="btn btn-sm btn-outline-secondary dropdown-toggle" data-bs-toggle="dropdown">Kelola</button><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="materi.php?pengajaran_id=<?= (int)$p['pengajaran_id'] ?>"><i class="fa-solid fa-book-open me-2 text-primary"></i>Materi</a></li><li><a class="dropdown-item" href="tugas.php?pengajaran_id=<?= (int)$p['pengajaran_id'] ?>"><i class="fa-solid fa-list-check me-2 text-warning"></i>Tugas</a></li><li><a class="dropdown-item" href="ujian.php?pengajaran_id=<?= (int)$p['pengajaran_id'] ?>"><i class="fa-solid fa-file-pen me-2 text-danger"></i>Ulangan Harian</a></li><li><a class="dropdown-item" href="rekap_nilai.php?pengajaran_id=<?= (int)$p['pengajaran_id'] ?>"><i class="fa-solid fa-chart-column me-2 text-success"></i>Rekap Nilai</a></li></ul></div></div><?php endforeach ?></div></details><?php endforeach ?></div><?php endif ?></div></section></div>

            <div class="col-lg-5 teacher-content-secondary" id="antrean-penilaian"><section class="card teacher-panel h-100"><div class="card-body p-4"><div class="dashboard-section-head d-flex justify-content-between align-items-center gap-2 mb-3"><div class="d-flex align-items-center gap-2"><span class="dashboard-section-icon bg-warning-subtle text-warning"><i class="fa-solid fa-clock-rotate-left"></i></span><div><h2 class="h6 fw-bold mb-0">Perlu Dinilai</h2><small class="text-muted"><?= count($recent_submissions) ?> terbaru</small></div></div><a href="tugas_penilaian.php" class="small text-decoration-none">Lihat semua</a></div>
                <?php if(!$recent_submissions): ?><div class="grading-empty"><div><span class="grading-empty-icon bg-success-subtle text-success"><i class="fa-solid fa-check"></i></span><p class="small mb-0">Semua tugas sudah dinilai.</p></div></div><?php else: ?><div class="grading-list"><?php foreach($recent_submissions as $sub): ?><article class="grading-compact"><span class="grading-compact-icon bg-warning-subtle text-warning"><i class="fa-solid fa-file-circle-check"></i></span><div class="item-copy flex-grow-1"><strong class="d-block small"><?= sanitize($sub['nama_siswa']) ?></strong><small class="text-muted d-block"><?= sanitize($sub['judul_tugas']) ?> &middot; <?= sanitize($sub['nama_kelas']) ?></small><small class="text-primary"><?= date('d M, H:i',strtotime($sub['dikumpulkan_pada'])) ?></small></div><a href="tugas_penilaian.php?id=<?= (int)$sub['pengumpulan_id'] ?>" class="btn btn-sm btn-outline-warning">Nilai</a></article><?php endforeach ?></div><?php endif; ?>
            </div></section></div>
        </div>
    </main>
</div>
