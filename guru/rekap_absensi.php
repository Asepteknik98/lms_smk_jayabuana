...
if ($pengajaran_id > 0) {
    ...
    $stmt = $db->prepare("SELECT s.nisn,s.nama_lengkap,
        ...
        SUM(CASE WHEN da.status='Terlambat' THEN 1 ELSE 0 END) terlambat,
    ...
    foreach ($rekap as &$row) {
        ...
        $row['persentase']=$total_sesi ? ($hadirEfektif/$total_sesi)*100 : 0;
    } unset($row);
}
...
<Row><?php foreach(['No','NISN','Nama Siswa','Hadir','Terlambat','Sakit','Izin','Alpa','Kehadiran (%)'] as $h): ?><Cell ss:StyleID="H"><Data ss:Type="String"><?= $xml($h) ?></Data></Cell><?php endforeach ?></Row>
<?php foreach($rekap as $i=>$r): ?><Row><Cell><Data ss:Type="Number"><?= $i+1 ?></Data></Cell><Cell><Data ss:Type="String"><?= $xml($r['nisn']) ?></Data></Cell><Cell><Data ss:Type="String"><?= $xml($r['nama_lengkap']) ?></Data></Cell><?php foreach(['hadir','terlambat','sakit','izin','alpa'] as $k): ?><Cell><Data ss:Type="Number"><?= (int)$r[$k] ?></Data></Cell><?php endforeach ?><Cell><Data ss:Type="Number"><?= number_format($r['persentase'],2,'.','') ?></Data></Cell></Row><?php endforeach ?>
    ...
    $lines=['REKAP ABSENSI SISWA - SMK JAYA BUANA',$info['nama_mapel'].' | '.$info['nama_kelas'].' | '.$info['nama_guru'],'Semester '.$info['semester'].' | '.$info['tahun_ajaran'].' | Total sesi: '.$total_sesi,str_repeat('-',112),sprintf('%-3s %-15s %-28s %5s %5s %5s %5s %5s %8s','No','NISN','Nama','H','T','S','I','A','Hadir %'),str_repeat('-',112)];
    foreach($rekap as $i=>$r){$nama=function_exists('mb_strimwidth')?mb_strimwidth($r['nama_lengkap'],0,28,'..','UTF-8'):substr($r['nama_lengkap'],0,28);$lines[]=sprintf('%-3d %-15s %-28s %5d %5d %5d %5d %5d %7.2f',($i+1),$r['nisn'],$nama,$r['hadir'],$r['terlambat'],$r['sakit'],$r['izin'],$r['alpa'],$r['persentase']);}
