...
<?php endif ?>
    ...
    else{$lines[]='Kode: H=Hadir, S=Sakit, I=Izin, A=Alpa, B=Belum check-in, -=Belum ada sesi';$lines[]=sprintf('%-3s %-13s %-20s %s %3s %3s %3s %3s %6s','No','NISN','Nama',implode(' ',array_map(static fn($p)=>sprintf('%02d',$p),range(1,20))),'H','S','I','A','%');$lines[]=str_repeat('-',112);foreach($rekap as $i=>$r){$nama=function_exists('mb_strimwidth')?mb_strimwidth($r['nama_lengkap'],0,20,'..','UTF-8'):substr($r['nama_lengkap'],0,20);$kode=[];for($p=1;$p<=20;$p++){$status=$status_pertemuan[(int)$r['id']][$p]??'-';$kode[]=in_array($status,['Hadir','Sakit','Izin','Alpa','Belum'],true)?$status[0]:'-';}$lines[]=sprintf('%-3d %-13s %-20s %s %3d %3d %3d %3d %6.1f',$i+1,$r['nisn'],$nama,implode('  ',$kode),$r['hadir'],$r['sakit'],$r['izin'],$r['alpa'],$r['persentase']);}}
